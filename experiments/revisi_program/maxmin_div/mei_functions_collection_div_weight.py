...
def calculate_set_utility(df,item,S_list):
    ...
    utility = (sum(df_util['Utility'])/k)/max_I
    return utility

def calculate_set_diversity(item,S_list):
    min_distance = 100
    for i in range(0, len(S_list)):
        d = diversity(item,S_list[i])
        if d < min_distance:
            min_distance = d
    return min_distance
...
def util_func(S_list):
    max_I = math.sqrt(2)
    new_df = pd.DataFrame(S_list)
    utility = (sum(new_df[4])/len(new_df))/max_I
    return utility
...
def util_compute(item, S_list, df):
    k = len(S_list) + 1
    max_I = math.sqrt(2)
    df_pg = pd.DataFrame(S_list, columns=['Attributes', 'Meassure', 'Function'])
    df_pg_result = df_pg.merge(df, how='left')
    util = ((sum(df_pg_result['Utility'])+max_I)/k)/max_I
    return util
