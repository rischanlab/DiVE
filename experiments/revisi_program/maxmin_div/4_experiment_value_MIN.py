...
def Only_Interestingness(df, k, output):
	...
	df_seedb = df_seedb.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_seedb.values.tolist()

	sum_div = div_func_d(S)
...
def Only_Diversity(df, k, output):
	...
	df_maxdiv = df_maxdiv.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_maxdiv.values.tolist()
...
def iDiVE_Greedy(df, k, output):
	...
	df_pg_result = df_pg_result.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_pg_result.values.tolist()
...
def iDiVE_SwapI(df, k, output):
	...
	df_swapU = df_swapU.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_swapU.values.tolist()
	sum_div = div_func_d(S)
	objf = sum_util+sum_div
	timeafter = datetime.datetime.now()    
	procesing_time = str(timeafter - timebefore)
...
def iDiVE_SwapD(df, k, output):
	...
	df_swapD = df_swapD.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_swapD.values.tolist()
	sum_div = div_func_d(S)
...
def iDiVE_SwapD_Pruning(df, k, output):
	...
	#print(df_swapD, file=open(output, "a"))
	df_swapD_Pruning = df_swapD_Pruning.drop(['Utility'],axis=1).reset_index(drop=True)
	S = df_swapD_Pruning.values.tolist()
	sum_div = div_func_d(S)
...
number_of_k = [5,10,15] #
output = "June_objf_disease_MaxMIN.csv"
n = len(df)
