...
def random_selection_return_value(df,k):
	df_random = df.sample(k)

	sum_util = (sum(df_random['Utility'])/k)/2
...
def random_selection(df,k, output):
	...
	for i in range(0,10):
		data = random_selection_return_value(df,k)
		result.append(data)

	df_rand = pd.DataFrame(result, columns=['Utility', 'Diversity', 'Objective'])

	sum_util = df_rand['Utility'].mean()
	sum_div = df_rand['Diversity'].mean()
	objf = df_rand['Objective'].mean()
	timeafter = datetime.datetime.now()    
	procesing_time = str(timeafter - timebefore)
	print("Random,{0},{1},{2},{3},{4}".format(k,sum_util,sum_div,objf,procesing_time), file=open(output, "a"))
